	...
	<script type="text/javascript">
			...
			var inputs = document.getElementsByTagName("input");
		    	...
		    	for(var i = 0 ;i<inputs.length;i++){
					if(inputs[i].type=="checkbox" ){
						if(inputs[i].checked){
							activityIds += "," + inputs[i].id;
						}
					}
				}
		    	 var pos=activityIds.lastIndexOf(",");
                var postfix = activityIds.substring(pos+1,activityIds.length);
                
                if(postfix==""){
               	 alert("请选择一条!");
               	 return false;
                }else{
               	 location.href="activityupdateactivity?id="+postfix;
                }
	   ...
	   function deleteactivity(){
			...
			var inputs = document.getElementsByTagName("input");
				...
				location.href="activitydeleteactivity?activityIds="+activityIds;
				...
				<s:if test='#session.user.duty.duty_ID!=5&&#session.user.duty.duty_ID!=4'>
					<div class="operate_div">
						<img src="image/0002.PNG"
						 ...
						 <span onclick="updateactivity();"><font color="black">修改信息</font></span>
					</div>
					<div class="operate_div">
						<img src="image/0003.PNG"
							style="margin:1px 5px 0px 4px; float: left;" />
						  <span onclick="deleteactivity();"><font color="black">删除信息</font></span>
					</div>
					</s:if>
					<div class="operate_div">
						<img src="image/0004.PNG"
							style="margin:1px 5px 0px 13px; float: left;" />
					    <span onclick="searchactivity();"><font color="black">查询信息</font></span>
					</div>
					...
					<table border="1" align="center" width="100%" class="datalist" id="tableid">
						<tr>
                            ...
                            <th>
								<font color="black" size="2">活动名称</font>
							</th>
							<th>
								<font color="black" size="2">所属团队</font>
							</th>
							<th>
								<font color="black" size="2">活动主题</font>
							</th>
							<th>
								<font color="black" size="2">举办日期</font>
							</th>
							<th>
								<font color="black" size="2">负责人</font>
							</th>
							<th>
								<font color="black" size="2">监查人</font>
							</th>
							<th>
								<font color="black" size="2">承办单位</font>
							</th>
							<th>
								<font color="black" size="2">活动内容</font>
							</th>
							<th>
						...
						<s:iterator value="activityList" var="activitys">
						<tr>
						    <td>
							     <input type="checkbox" id="<s:property value="#activitys.activity_id" />" >
							</td>
							<td>
							<font color="black"><s:property value="#activitys.activity_name" /></font>
							</td>
							<td>
							    <s:property value="#activitys.activity_community" />
							</td>
							<td>
							    <s:property value="#activitys.activity_theme" />
							</td>
							<td>
							    <s:date name="#activitys.activitydate" format="yyyy-MM-dd"/>
							</td>
							<td>
								<s:property value="#activitys.principal" />
							</td>
							<td>
								<s:property value="#activitys.hkcss_inspection" />
							</td>
							<td>
								<s:property value="#activitys.unit" />
							</td>
							<td>
							    <s:property value="#activitys.activity_content" />
							</td>
							<td>
								<s:property value="#activitys.note" />
							</td>
